 ...
 */
class Deconfin_Logger_File extends Deconfin_Logger_Abstract
{
  /**
  ...
  public function __construct(Deconfin_Client $client)
  {
    ...
    $file = $client->getClassConfig('Deconfin_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('Deconfin_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('Deconfin_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new Deconfin_Logger_Exception('File pointer is no longer available');
    }
