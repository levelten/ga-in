 ...
 */
class Deconfin_Signer_P12 extends Deconfin_Signer_Abstract
{
  ...
  public function __construct($p12, $password)
  {
    ...
    if (!$password && strpos($p12, "-----BEGIN RSA PRIVATE KEY-----") !== false) {
      ...
      if (!array_key_exists("pkey", $certs) || !$certs["pkey"]) {
        throw new Deconfin_Auth_Exception("No private key found in p12 file.");
      }
      $this->privateKey = openssl_pkey_get_private($certs['pkey']);
    }

    if (!$this->privateKey) {
      throw new Deconfin_Auth_Exception("Unable to load private key");
    }
  }
  ...
  public function sign($data)
  {
    ...
    if (!openssl_sign($data, $signature, $this->privateKey, $hash)) {
      throw new Deconfin_Auth_Exception("Unable to sign data");
    }
    return $signature;
  }
}
