...
class Deconfin_Task_Runner
{
  ...
  /**
   * @var Deconfin_Client $client The current API client.
   */
  private $client;
   ...
   *
   * @param Deconfin_Client $client The current API client
   * @param string $name The name of the current task (used for logging)
   * @param callable $action The task to run and possibly retry
   * @param array $arguments The task arguments
   * @throws Deconfin_Task_Exception when misconfigured
   */
  public function __construct(
    ...
    $config = (array) $client->getClassConfig('Deconfin_Task_Runner');

    if (isset($config['initial_delay'])) {
      if ($config['initial_delay'] < 0) {
        throw new Deconfin_Task_Exception(
   ...
   *
   * @return mixed
   * @throws Deconfin_Task_Retryable on failure when no retries are available.
   */
  public function run()
  {
    while ($this->attempt()) {
      ...
      } catch (Deconfin_Task_Retryable $exception) {
